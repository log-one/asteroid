...
io.use(async function (socket, next) {
  ...
  //listener for when user tries to befriend someone by saying #ily
  socket.on("#ily", async ({ userName, lastMessage }) => {
    //get user document
    let user = await getUser(userName);
    //get peer name from room name
    let peerName = getPeerFromRoom(user.currentRoom, user.name);

    //emit #ily message to room
    io.in(user.currentRoom).emit("message", {
      user: `${user.name}`,
      text: `#ily`,
    });
    ...
    //if #ily was said immediately after peer said it, and they are not already friends, form the union and pronounce them friends
    if (
    ...
    //if they are already friends but still used the #ily commands, emit a silly admin message
      ...
      //if this is first user to say #ily and extend an invitation to be friends
